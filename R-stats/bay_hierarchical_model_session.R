...
df <- read_excel("Y:/raw-imaging/Adrianna/experiments/analysis/Adrianna/PN/excels/spontaneous_summary_data_PN.xlsx", sheet = 1)
df$virus <- relevel(factor(df$virus), ref = "sgRosa")
...
model <- brm(
  mean_corr_coef ~ virus + (1 | subjects), 
  data = df, 
  family = gaussian(),
  chains = 4, cores = 4,
  ...
  prior = priors,
  control = list(
    adapt_delta = 0.99, # Force des pas d'échantillonnage plus petits (par défaut 0.80)
