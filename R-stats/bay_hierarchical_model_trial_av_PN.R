...
df <- read_excel("Y:/raw-imaging/Adrianna/experiments/analysis/Adrianna/PN/excels/Natural-Images-4-repeats_summary_data_rois_PN.xlsx", sheet = 1)
...
priors <- c(
  prior(normal(0, 0.5), class = "b"),               # Effet du génotype
  prior(exponential(2), class = "sd"),            # Restreint la variance des individus/sessions
  prior(exponential(2), class = "sigma")          # Bruit résiduel des cellules
)
...
model <- brm(
  ...
  warmup = 1000,
  control = list(
    adapt_delta = 0.995, # Force des pas d'échantillonnage plus petits (par défaut 0.80)
