            ...
            for state, state_filter in zip(states_names, states_filters):
                ...
                for roi_idx in range(len(pos_evokedStats['significant'])):
                    ...
                    if pos_evokedStats['significant'][roi_idx]:
                        ...
                        DATASET_ROIS['mean_avg_trial_win'].append(np.mean(ep.dFoF[state_filter][:, roi_idx, :][:, window]) - roi_bsl)
                        ...
                        DATASET_ROIS['mean_avg_trial_win'].append(np.nan)

            ###############################
        
        else :
            DATASET["nb_responsive_neurons_pos"].append(np.nan)
            DATASET["nb_responsive_neurons_neg"].append(np.nan)
            DATASET['mean_dFoF_pos'].append(np.nan)
            DATASET['mean_dFoF_neg'].append(np.nan)

    else :
        print("session %s has no ROIs, excluded from analysis" % filename)
        DATASET["nb_responsive_neurons_pos"].append(0)
        DATASET["nb_responsive_neurons_neg"].append(0)
        DATASET['mean_dFoF_pos'].append(np.nan)
        DATASET['mean_dFoF_neg'].append(np.nan)
