...
for i, filename in enumerate(DATASET['files']):
    ...
    if data.nROIs>0:
        ...
        if 'angle' in ep.varied_parameters:
                fig, AX = physion.dataviz.episodes.trial_average.plot(ep,
                                                                ...
                                                                **plot_props)
                for i in range(2):
                        image_cond = ep.find_episode_cond(key='angle', index=i)
...
for j, cond in enumerate(['all', 'run', 'still']):
    for i, c in zip(range(2), ep.varied_parameters['angle']):
