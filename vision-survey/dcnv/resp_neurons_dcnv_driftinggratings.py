...
for virus in ['sgRosa', 'sgCnr1']:
      for cond in ['all', 'run', 'still']:
              for c, contrast in enumerate([0.2, 0.6, 1.0]):
                pos_means['%s-%s-c=%.1f' % (virus, cond, contrast)] = []

neg_means = {} # 
for virus in ['sgRosa', 'sgCnr1']:
      for cond in ['all', 'run', 'still']:
              for c, contrast in enumerate([0.2, 0.6, 1.0]):
                neg_means['%s-%s-c=%.1f' % (virus, cond, contrast)] = []
...
for virus in ['sgRosa', 'sgCnr1']:
        ...
        for c, contrast in enumerate([0.2, 0.6, 1.0]):
                pos_percentages['%s-c=%.1f' % (virus, contrast)] = []
...
for virus in ['sgRosa', 'sgCnr1']:
        ...
        for c, contrast in enumerate([0.2, 0.6, 1.0]):
                neg_percentages['%s-c=%.1f' % (virus, contrast)] = []
...
for virus in ['sgRosa', 'sgCnr1']:
      for cond in ['all', 'run', 'still']:
        for c, contrast in enumerate([0.2, 0.6, 1.0]):
                ...
                run_means['%s-%s-c=%.1f' % (virus, cond, contrast)] =[]
                ...
                for cond, filter in zip(['all', 'run', 'still'],
                        ...
                        if (np.sum(responsiveROIs)>=NMIN_ROIS) and \
                                        ...
                                        means['%s-%s-c=%.1f' % (virus, cond, contrast)].append(
                                                epGrating.Deconvolved[contrast_cond & filter, :, :][:, responsiveROIs, :])
                
                        
                        pos_means['%s-%s-c=%.1f' % (virus, cond, contrast)].append(
                                        epGrating.Deconvolved[contrast_cond & filter, :, :][:, pos_responsiveROIs, :])
                        neg_means['%s-%s-c=%.1f' % (virus, cond, contrast)].append(
                                        epGrating.Deconvolved[contrast_cond & filter, :, :][:, neg_responsiveROIs, :])
...
firgurename = 'dcnv_beh_mod_drifting_'+ neuron + '.svg'
...
#%%
firgurename = 'dcnv_beh_mod_onlypos_'+ neuron + '.svg'
fig, AX = pt.figure(axes=(3,3))
