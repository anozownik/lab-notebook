...
plot_props = dict(column_key='speed',
...
for key in ['sgRosa', 'sgCnr1']:
      means['all-%s' % key] = [[] for i in range(2)]
      means['run-%s' % key] = [[] for i in range(2)]
      means['still-%s' % key] = [[] for i in range(2)]
    ...
    if data.nROIs>0:
        ...
        if 'speed' in ep.varied_parameters:
                fig, AX = physion.dataviz.episodes.trial_average.plot(ep,
                                                                ...
                                                                **plot_props)
                for i in range(2):
                        image_cond = ep.find_episode_cond(key='speed', index=i)
...
for j, cond in enumerate(['all', 'run', 'still']):
    for i, c in zip(range(2), ep.varied_parameters['speed']):
