...
stat_test_props['interval_post'] = [1, 2]
...
for i, filename in enumerate(DATASET['files']):
    ...
    #date condition
    #if data.nwbfile.session_start_time.date() >= datetime.date(2026, 1, 1) and  data.nwbfile.session_start_time.date() <= datetime.date(2026, 4, 1) and virus == 'sgRosa':
...
fig, AX = pt_fcts.plot_average_response(ep.t, pos_means, 
                                        viruses, states_names, vparam_values, varied_parameter, 
                                        included_mice, params.NMIN_SESSIONS, 
                                        baselineSubtraction=False, baselineCond=baselineCond)
...
for vparam in vparam_values:
    ...
    AX.set_ylabel('gain')
    AX.set_title(f'contrast {vparam:.2f}', y=1.15)
